...
pub fn number(slice: &mut Peekable<&mut Chars>) -> Result<Json, Error> {
    ...
    'tokenizer: loop {
        ...
        match stage {
            ...
            Stages::Sign => {
                match current {
                    '-' => {
                        stage = Stages::AfterSign;
                        is_unsigned = false;
                        token.push(current);
                        slice.next();
                    }
                    '0' => {
                        stage = Stages::AfterZero;
                        token.push(current);
                        slice.next();
                    }
                    '1'...'9' => {
                        stage = Stages::Integer;
                        token.push(current);
                        slice.next();
                    }
            ...
            Stages::AfterSign => {
                match current {
                    ...
                    '.' => {
                        stage = Stages::AfterDot;
                        token.push(current);
                        slice.next();
                    }
                    'e' | 'E' => {
                        stage = Stages::AfterExp;
                        token.push(current);
                        slice.next();
                    }
    ...
    Ok(Json::Number(match stage {
        ...
        _ => Number::Float(token.parse::<f64>().unwrap()),
    }))
}
